...
function find_cni_conf_file() {
    ...
    for cfgf in "${MOUNTED_CNI_NET_DIR}"/*; do
        if [ "${cfgf: -5}" == ".conf" ]; then
            ...
            type=$(jq 'has("type")' < "${MOUNTED_CNI_NET_DIR}/${cfgf}" 2>/dev/null)
            if [[ "${type}" == "true" ]]; then
                cni_cfg=${cfgf}
                break
            fi
            ...
            name=$(jq 'has("name")' < "${MOUNTED_CNI_NET_DIR}/${cfgf}" 2>/dev/null)
            plugins=$(jq 'has("plugins")' < "${MOUNTED_CNI_NET_DIR}/${cfgf}" 2>/dev/null)
...
if [ -e "${MOUNTED_CNI_NET_DIR}/${CNI_CONF_NAME}" ]; then
    ...
    CNI_CONF_DATA=$(jq --argjson CNI_TMP_CONF_DATA "$CNI_TMP_CONF_DATA" -f /filter.jq < "${CNI_CONF_FILE}")
